...
public class Ia {
	...
	public BufferedImage[] loadCommandes() throws AWTException {
		...
		for (int i = 1; i <= 6; i++) {
			sprites[i - 1] = new Robot().createScreenCapture(commandes.get("Bulle" + Integer.toString(i)));
		}
	...
	public void saveSprites() throws AWTException, IOException {
		...
		for (int i = 1; i <= 6; i++) {
			...
			Rectangle rec = this.commandes.get(bubbleName);
			BufferedImage img = new Robot().createScreenCapture(rec);
			ImageIO.write(img, "png", new File(System.getProperty("user.dir") + "/screenshots/" + bubbleName + new Date().getTime() + ".png"));
	 ...
	 * @param height
	 */
	public void fillGreyTable(String name, Dot hautGauche, double width, double height) {

		greyIngredients.put(name, new Rectangle(hautGauche.x, hautGauche.y, (int) width, (int) height));
	 ...
	 * @param height
	 */
	public void fillCommandes(String name, Dot hautGauche, double width, double height) {

		commandes.put(name, new Rectangle(hautGauche.x, hautGauche.y, (int) width, (int) height));
	...
	public void useIngredient(String s) throws Exception {
		...
		if (ingredients.containsKey(s)) {
			...
			throw new Exception("Ingredient non pr��sent dans la liste.");
		}
